...
def decode_and_collect(decoder, z, y, save_dir, args, ptdtype, device, H_grid: int, W_grid: int):
    if args.save_images:
...
def run_metrics(input1_src, args, eval_name, reference_input2, cache_input2_name=None):
    """Invokes torch_fidelity FID evaluation."""
    logger.info(f"Running FID evaluation for: {eval_name}")
    ...
    logger.info(f"[{eval_name}] FID: {metrics}")
    return metrics
...
def main(cli_args):
    ...
    if args.dataset_name == "cifar-10":
        cifar_root = "workspace/datasets/cifar-10"
        cifar_archive = osp.join(cifar_root, "cifar-10-python.tar.gz")
        already_exists = osp.isfile(cifar_archive) or osp.isdir(osp.join(cifar_root, "cifar-10-batches-py"))
        if already_exists:
            logger.info("CIFAR-10 dataset found locally, skipping download.")
        else:
            logger.info("CIFAR-10 not found locally, downloading...")
        real_reference_dataset = torchvision.datasets.CIFAR10(
            root=cifar_root, train=True, download=not already_exists
        )
    ...
    for split_id, split_name in splits_to_eval:
        ...
        overall_cache  = f"{args.dataset_name}-train" if args.dataset_name == "cifar-10" else None
        metric_input_overall = split_dir if args.save_images else overall_split_ds

        metrics_log_book[split_name] = run_metrics(
            metric_input_overall, args, split_name, overall_input2,
            cache_input2_name=overall_cache,
        )

    print("\n" + "="*50 + "COMPREHENSIVE GENERATION METRICS REPORT\n" + "="*50)
