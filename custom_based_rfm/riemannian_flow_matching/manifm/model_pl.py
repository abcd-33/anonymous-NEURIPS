...
class ManifoldFMLitModule(pl.LightningModule):
    ...
    def rfm_loss_fn(self, batch: torch.Tensor):
        """Sequence-preserving Riemannian Flow Matching Loss."""
        if isinstance(batch, dict):
            x1 = batch["x1"]  
            y = batch["y"]    
        else:
            x1 = batch        
            y = torch.zeros(x1.shape[0], dtype=torch.long, device=x1.device)

        if x1.shape[-1] != self.dim and x1.shape[1] == self.dim:
            x1 = x1.permute(0, 2, 1).contiguous()

        B, S, D = x1.shape 

        if isinstance(self.manifold, Mesh) or isinstance(self.manifold, SPD):
            raise NotImplementedError("Sequence-aware flow matching is not yet implemented for Mesh/SPD manifolds.")
            
        else:
            x0_uniform = self.manifold.random_base(B * S, self.dim).to(x1).view(B, S, D)

            mix_mask = torch.rand(B, 1, 1, device=x1.device) > 0.5  
            random_sigmas = torch.rand(B, 1, 1, device=x1.device) * 0.5
            noise = torch.randn_like(x1) * random_sigmas
            
            x0_local = self.manifold.expmap(x0_uniform.view(-1, D), noise.view(-1, D)).view(B, S, D)
            x0 = torch.where(mix_mask, x0_uniform, x0_local)

            beta_dist = torch.distributions.Beta(1, 2)
            t_batch = beta_dist.sample((B, 1)).to(x1.device) 

            is_local = ~mix_mask.squeeze(-1).squeeze(-1) 
            if is_local.any():
                t_batch[is_local] = 0.4 + (1.0 - 0.4) * t_batch[is_local]

            t_seq = t_batch.unsqueeze(1).expand(-1, S, -1)

            def cond_u(x0_tok, x1_tok, t_tok):
                path = geodesic(self.manifold, x0_tok, x1_tok)
                x_t_tok, u_t_tok = jvp(path, (t_tok,), (torch.ones_like(t_tok),))
                return x_t_tok, u_t_tok

            x_t, u_t = vmap(cond_u)(x0.reshape(-1, D), x1.reshape(-1, D), t_seq.reshape(-1, 1))
            u_t = torch.clamp(u_t, min=-20.0, max=20.0)

            x_t = x_t.reshape(B, S, D)
            u_t = u_t.reshape(B, S, D)

            # Pass 1D tensor [B] down to the vecfield wrapper natively
            t_pass = t_batch.squeeze(-1) 

            v_pred = self.vecfield(t_pass, x_t, y)
            
            diff = v_pred - u_t
            
            loss = self.manifold.inner(x_t.view(-1, D), diff.view(-1, D), diff.view(-1, D))
            loss = loss.view(B, S)
            loss = loss * (2 - t_seq.squeeze(-1))**2
    ...
    def configure_optimizers(self):
        ...
        if self.cfg.optim.get("scheduler", "cosine") == "cosine":
            ...
            warmup_steps = self.cfg.optim.get("warmup_steps", 500)
    ...
    def on_before_optimizer_step(self, optimizer):
        # This hook runs AFTER configure_gradient_clipping() has executed.
        # If you measure the norms here, you will see them capped at 1.0.
        ...
        if self.global_step % 50 == 0:
            self.log("grad_norm/post_clip", total_norm, on_step=True)

        # This dictionary will hold norms grouped by input concepts
        pathway_norms = {
            "hypersphere_layers": [],
            "conditioning_layers": [] # time, class, token
        }
        ...
        for name, param in self.named_parameters():
            if param.requires_grad and param.grad is not None:
                ...
                # Calculate the Step-to-Weight ratio we talked about
                ratio = grad_norm / (weight_norm + 1e-8)
                
                # Route them based on your internal naming conventions
                # Adjust these strings to match your actual layer names!
                if "sphere" in name or "input_layer" in name:
                    pathway_norms["hypersphere_layers"].append(grad_norm)
                elif any(k in name for k in ["time", "token", "class", "cond"]):
                    pathway_norms["conditioning_layers"].append(grad_norm)
                    
                # Log individual layer ratios to your logger
                if self.global_step % 50 == 0: # Don't spam every single step
                    self.log(f"ratio/{name}", ratio, on_step=True, on_epoch=False)

        # Average out the pathways to check for absolute dominance
        if self.global_step % 50 == 0:
            for pathway, norms in pathway_norms.items():
                if norms:
                    avg_norm = sum(norms) / len(norms)
                    self.log(f"pathway_norm/{pathway}", avg_norm, on_step=True)
